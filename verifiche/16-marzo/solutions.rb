class User
  ...
  def errors
    ...
    e << :email if (@email.class != String || @email.empty?)
    e << :name if (@name.class != String || @name.empty?)
    e << :surname if (@surname.class != String || @surname.empty?)
    e << :sign_in_count if (@sign_in_count.class != Fixnum || @sign_in_count < 0)
    e << :registration_date if (@registration_date.class != Date)
...
class ProblemA
  ...
  def self.sum_fixnums(input)
    input.select { |e| e.class == Fixnum }.inject(:+) || 0
  end
end
